import { Interaction, SlashCommandBuilder, TextBasedChannel } from "discord.js";
import {
  beginJudging,
...
import { sendMessageToChannel } from "../util/shardMessaging.js";
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option
        .setName("number")
        .setDescription("Number of the card you would like to submit")
        .setMinValue(1)
        .setRequired(true)
    )
  ...
  async execute(interaction: Interaction) {
    ...
    if (!interaction.channel || !interaction.channel.isDMBased())
      throw new CAHError(
        "This command can only be executed in a direct message channel."
      );
    ...
    if (data.allSubmitted) {
      ...
      scheduleJob(data.resultDisplayTime, async () => {
        try {
          axios
            .post(config.apiEndpoint + "/bot/game/endRound", {
              gameId: data.gameId,
            })
            .then(async (res) => {
              const data = res.data;
          ...
          console.error(
            "A scheduler error occurred (endpoint: endRound). Error has been reported to sentry."
          );
